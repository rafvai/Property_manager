    ...
    try:
        conn.execute(
            """INSERT INTO users (email, password_hash, full_name, plan, expires_at)
               VALUES (?,?,?,?,?)""",
            (user.email.lower().strip(), hash_password(user.password),
             user.full_name, user.plan, expires)
        )
        conn.commit()
        row = conn.execute(
    ...
    return [
        {
            ...
            "days_left"   : days_left(r["expires_at"]),
...
async def upload_translations(file: UploadFile):
    """Upload nuovo file traduzioni (solo admin). UploadFile importato in cima al file."""
