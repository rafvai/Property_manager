...
if __name__ == "__main__":
    print("=== TEST VALIDAZIONE SICURA ===\n")

    # Test parse_decimal
    print("1. Test parse_decimal:")
    try:
        val = parse_decimal("1.234,56")
        print(f"   ✅ '1.234,56' -> {val}")
    except ValidationError as e:
        print(f"   ❌ {e}")

    try:
        val = parse_decimal("999999999999")
        print(f"   ❌ Valore troppo grande dovrebbe fallire!")
    except ValidationError as e:
        print(f"   ✅ Bloccato: {e}")
    ...
    try:
        validate_required_text("'; DROP TABLE users--", "Campo")
        print("   ❌ SQL injection NON rilevata!")
    except ValidationError as e:
        print(f"   ✅ Bloccato: {e}")
    ...
    print("\n3. Test XSS:")
    try:
        clean = validate_required_text("<script>alert('XSS')</script>", "Campo")
        print(f"   ✅ Sanitizzato: '{clean}'")
    except ValidationError as e:
        print(f"   ⚠️  {e}")

    # Test metadati
    print("\n4. Test validazione metadati:")
    metadata = {
        'tipo': 'Entrata',
    ...
    try:
        ...
        print(f"   ✅ Metadati validati: {validated}")
    except ValidationError as e:
        print(f"   ❌ {e}")